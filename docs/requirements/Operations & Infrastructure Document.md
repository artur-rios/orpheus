...
**IR-18** — Neither a failing analyzer nor a failing test suite may be released.

**IR-19** — The Linux build requires `libmpv` at runtime on the target machine.
...
**IR-20** — Interactive controls carry labels, and the reduced-motion setting is
honoured. The reduced-motion path is the one the widget tests exercise, so it is
the path that stays correct.

**IR-21** — The arrangement follows window width at every size, not the
