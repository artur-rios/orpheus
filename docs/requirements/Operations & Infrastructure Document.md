...
**IR-18** — Verification is a script, not a remembered sequence: `tools/verify.sh`
...
**IR-19** — Because no single host can build every target, the claim is only
...
**IR-20** — The workflow reads the built Android package's permissions back out
...
**IR-21** — Neither a failing analyzer nor a failing test suite may be released.

**IR-22** — The Windows installer is produced by Inno Setup from
...
**IR-23** — Installing, upgrading and uninstalling never touch the catalog, the
cover cache, the play history or the settings. An uninstall is not a request to
forget what the owner listened to.

**IR-24** — The installer is unsigned, and this is a deliberate deferral rather
...
**IR-25** — The Linux build requires `libmpv` at runtime on the target machine.
...
**IR-26** — Interactive controls carry labels, and the reduced-motion setting is
honoured. The reduced-motion path is the one the widget tests exercise, so it is
the path that stays correct.

**IR-27** — The arrangement follows window width at every size, not the
